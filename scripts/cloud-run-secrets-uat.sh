#!/usr/bin/env bash
# Usage: ./scripts/cloud-run-secrets-uat.sh <service-name>
# Outputs comma-separated ENV=SECRET:VERSION for --set-secrets (uat)
# NOTE: Shares DB/Kafka secrets with dev — separate later when UAT data isolation is needed.
