          ...
          <div className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            {data.summary.map((item, idx) => (
              ...
              >
                ...
                <p className={`mt-1 text-xs font-medium ${item.up ? 'text-emerald-500' : 'text-rose-400'} ${idx === 0 ? '' : ''}`}>
                  {item.up ? '▲' : '▼'} {item.change}
                </p>
              </article>
            ))}
            ...
            return (
              <div key={d.day} className="group flex flex-1 flex-col items-center gap-2">
                <div className="relative w-full">
                  <div
                    className="w-full rounded-t-[10px] bg-[#dbeafe] transition-all group-hover:bg-[#93c5fd]"
                    style={{ height: `${Math.max(pct * 1.4, 4)}px` }}
                  />
                  <div
                    className="absolute bottom-0 left-0 w-full rounded-t-[10px] bg-[#2563eb] transition-all group-hover:bg-[#1d4ed8]"
                    style={{ height: `${Math.max(pct * 1.4 * 0.55, 2)}px` }}
                  />
                </div>
                ...
                <div className="text-right">
                  ...
                  <div className="mt-1 h-1 w-16 overflow-hidden rounded-full bg-[#dbeafe]">
                    <div
                      ...
                      style={{ width: `${Math.round((p.revenue / (data.topProducts[0]?.revenue ?? 1)) * 100)}%` }}
                    />
                  </div>
                </div>
              </div>
